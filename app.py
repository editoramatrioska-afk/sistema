...
def login():
    ...
    with col_c:
        ...
        st.subheader("Editora Matrioska")
...
# --- 3. FUNÇÕES DO SISTEMA (PDF E CÁLCULOS) ---
...
def gerar_pdf_matrioska(dados):
    ...
    texto_apresentacao = (
        ...
        "Nosso catálogo contempla autoras e autores de primeira linha...\n\n"
...
# --- 4. LÓGICA PRINCIPAL (LOGIN OU SISTEMA) ---
...
if not st.session_state['logged_in']:
    ...
    with col1:
        ...
        total_caracteres = contar_caracteres_oficial_word(arquivo_word) if arquivo_word else st.number_input("Caracteres:", value=0)
    ...
    with col2:
        ...
        formato_sel = st.selectbox("Formato:", opcoes_formato)
    ...
    if total_caracteres > 0:
        ...
        if st.button("💾 Salvar no Banco"):
            payload = {"cliente": nome_cliente, "valor_total": valor_total, "livro": nome_livro}
            supabase.table("orcamentos").insert(payload).execute()
            st.success("Salvo!")

        try:
            dados = {"cliente": nome_cliente, "livro": nome_livro, "laudas": qtd_laudas, 
                     "formato": formato_sel, "miolo": miolo_sel, "capa": capa_sel, 
                     "acabamento": acabamento_sel, "paginas": total_paginas, "total": valor_total}
            pdf_bytes = gerar_pdf_matrioska(dados)
            st.download_button("📥 Baixar PDF", pdf_bytes, f"Proposta_{nome_livro}.pdf", "application/pdf")
